...
DEPLOY_DIR="/opt/career-plan/production"
JAR_FILE="../backend/target/career-plan-2026-1.0.0.jar"
PORT=9999

echo "======================================"
echo "🚀 Production 环境部署开始"
echo "======================================"
echo "⚠️  警告：这是生产环境部署！"
echo "⚠️  按 Ctrl+C 取消，或等待 5 秒继续..."
sleep 5

# 验证 JAR 文件存在
if [ ! -f "$JAR_FILE" ]; then
    echo "❌ JAR 文件不存在：$JAR_FILE"
    echo "💡 请先运行：./build-prod.sh"
    exit 1
fi
...
if [ -f career-plan-2026-1.0.0.jar ]; then
...
nohup java -Xms1g -Xmx4g \
  ...
  -XX:HeapDumpPath=/opt/career-plan/production/heapdump.hprof \
  -jar career-plan-2026-1.0.0.jar \
  --spring.profiles.active=production > app.log 2>&1 &
EOF
...
echo "🌐 访问地址：https://plan.shujuyunxiang.com/back-server"
echo "======================================"
