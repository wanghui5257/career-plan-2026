# 后端开发计划 - backend-dev

## 📋 角色职责

**后端开发工程师** - 负责职业发展计划系统的后端开发

---

## 🎯 当前任务

### Task 1: 后端 API 开发 ✅

**状态**: 已完成  
**完成时间**: 2026-03-12

**内容**:
- ✅ SpringBoot 项目搭建
- ✅ 用户认证接口（登录/退出）
- ✅ 任务管理接口（CRUD）
- ✅ 进度报告接口
- ✅ 健康检查接口
- ✅ API 文档（Swagger）

**提交记录**:
-  Merge branch feature/backend-dev into main
-  test: 修复测试配置
-  test: 添加测试数据库 schema
-  test: 添加 API 接口单元测试
-  fix: 修复 TaskController 路径为 /api/v1/tasks

---

### Task 2: 数据库设计 ✅

**状态**: 已完成  
**完成时间**: 2026-03-12

**内容**:
- ✅ 用户表（users）
- ✅ 任务表（tasks）
- ✅ 进度报告表（progress_reports）
- ✅ 测试数据初始化

**表结构**:


---

### Task 3: 服务器部署 ✅

**状态**: 已完成  
**完成时间**: 2026-03-13

**内容**:
- ✅ 阿里云 ECS 配置
- ✅ SSH 密钥认证
- ✅ Nginx 反向代理配置
- ✅ 生产环境部署
- ✅ 日志管理

**部署信息**:
- **服务器**: 47.115.63.159
- **端口**: 9999
- **Context Path**: /back-server
- **部署目录**: /opt/career-plan/

---

## 📅 明日计划

### Task 4: 功能增强

**计划时间**: 2026-03-14

**内容**:
- [ ] 任务分类/标签功能
- [ ] 任务搜索/过滤
- [ ] 用户个人资料管理
- [ ] 密码修改功能
- [ ] 数据导出功能

---

### Task 5: 性能优化

**计划时间**: 2026-03-15

**内容**:
- [ ] 数据库查询优化
- [ ] 接口响应时间优化
- [ ] 缓存机制
- [ ] 日志优化

---

## 📊 技术栈

| 技术 | 版本 | 用途 |
|------|------|------|
| Java | 17 | 开发语言 |
| Spring Boot | 3.2.3 | Web 框架 |
| Spring Security | 6.x | 安全认证 |
| MySQL | 8.0 | 数据库 |
| JPA/Hibernate | 6.x | ORM |
| Maven | 3.8.7 | 构建工具 |
| Swagger | 3.x | API 文档 |

---

## 🔗 相关链接

- **Swagger UI**: http://47.115.63.159:9999/back-server/swagger-ui.html
- **API Docs**: http://47.115.63.159:9999/back-server/api-docs
- **GitHub**: https://github.com/wanghui5257/career-plan-2026/tree/feature/backend-dev

---

**最后更新**: 2026-03-13 18:07 GMT+8

---

## 🆕 新增需求 (2026-03-13)

### Feature 1: 移动端响应式支持 📱

**优先级**: 高  
**计划时间**: 2026-03-14

**需求描述**:
- 计划看板需要支持手机显示
- 响应式布局适配各种屏幕尺寸
- 移动端友好的交互设计

**技术方案**:
- 使用 Ant Design Mobile 组件库
- CSS Media Queries 响应式布局
- 触摸友好的交互设计
- 测试设备：iOS Safari, Android Chrome

**验收标准**:
- [ ] 手机屏幕（375x667）正常显示
- [ ] 平板屏幕（768x1024）正常显示
- [ ] 触摸操作流畅
- [ ] 加载速度优化

---

### Feature 2: 总体目标和进度展示 📊

**优先级**: 高  
**计划时间**: 2026-03-14

**需求描述**:
- 首页展示总体目标和进度
- 分项目标和进度展示
- 可视化进度图表

**技术方案**:
- 新增目标管理模块
- 进度计算逻辑（按任务完成情况）
- 使用 Charts 组件展示进度
- 后端新增目标/进度 API

**API 设计**:


**验收标准**:
- [ ] 首页显示总体目标
- [ ] 进度百分比正确计算
- [ ] 分项目标清晰展示
- [ ] 进度图表直观易懂

---

### Feature 3: 任务 Markdown 文档支持 📝

**优先级**: 高  
**计划时间**: 2026-03-15

**需求描述**:
- 任务需要输出文档时支持 Markdown 输入
- 支持 Markdown 文档显示
- 文档版本管理

**技术方案**:
- 前端集成 Markdown 编辑器（如 @uiw/react-md-editor）
- 后端新增文档存储字段
- Markdown 渲染组件
- 文档版本记录

**数据库变更**:


**API 设计**:


**验收标准**:
- [ ] Markdown 编辑器正常显示
- [ ] Markdown 渲染正确
- [ ] 文档保存成功
- [ ] 版本历史记录完整

---

## 📋 开发优先级

| 优先级 | 功能 | 计划时间 | 状态 |
|--------|------|----------|------|
| P0 | 移动端响应式支持 | 2026-03-14 | ⏳ 待开发 |
| P0 | 总体目标和进度展示 | 2026-03-14 | ⏳ 待开发 |
| P1 | 任务 Markdown 文档支持 | 2026-03-15 | ⏳ 待开发 |
| P2 | 任务分类/标签功能 | 2026-03-14 | ⏳ 待开发 |
| P2 | 性能优化 | 2026-03-15 | ⏳ 待开发 |

---

**最后更新**: 2026-03-13 18:15 GMT+8

---

## 新增功能需求 (2026-03-13 18:30)

### Feature 4: 计划和任务 API 录入

**优先级**: P0 高  
**计划时间**: 2026-03-14

**需求描述**:
- 后端提供 API 接口录入计划和任务
- 支持 Worker 通过 API 提交目标和计划任务
- 支持批量导入任务

**API 设计**:
- POST /api/v1/plans - 创建计划
- POST /api/v1/plans/{id}/tasks - 添加任务
- POST /api/v1/plans/bulk - 批量导入

---

### Feature 5: 角色权限管理

**优先级**: P0 高  
**计划时间**: 2026-03-14

**角色定义**:
- 计划录入者 (PLAN_CREATOR): 创建计划、分配任务
- 监督执行者 (SUPERVISOR): 查看进度、发送提醒
- 任务执行者 (EXECUTOR): 查看任务、更新进度

**技术方案**:
- 数据库新增角色表
- 基于角色的权限控制 (RBAC)
- JWT Token 包含角色信息

---

### Feature 6: 提醒和通知系统

**优先级**: P1 中  
**计划时间**: 2026-03-15

**需求描述**:
- 监督执行者发送提醒
- 任务到期自动提醒
- 提醒历史记录

---

## 更新后的开发优先级

| 优先级 | 功能 | 时间 | 状态 |
|--------|------|------|------|
| P0 | 移动端响应式 | 03-14 | 待开发 |
| P0 | 目标进度展示 | 03-14 | 待开发 |
| P0 | API 录入 | 03-14 | 新增 |
| P0 | 角色权限 | 03-14 | 新增 |
| P1 | Markdown 文档 | 03-15 | 待开发 |
| P1 | 提醒系统 | 03-15 | 新增 |

